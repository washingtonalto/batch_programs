...
Copyright    : Copyright (c) 2007, Washington Alto. All rights reserved.
Date Started : July 2006
Date Modified: August 1, 2007
...
Parameters   :  [JDBC URL]            - JDBC URL
 ...
 Functions Calls:
   ...
   static String[] getFormatFileInput(String InputFormatFile,boolean IsCSV)
   ...
   static String padFieldNames (String inputFieldString,String padStrField,int rowNum,String FieldName,boolean IsFieldNull) {
 Modification History:
*  2007-05-30 - Added 3rd parameter in padspaces specification file for validating CHAR fields. The fields are
*               validated either of the ff: ALPHA, ALPHANUMERIC and NUMERIC. If field value is not valid, then a
*               blank or empty string is returned.
*               Added 4th parameter in padspaces specification file for replacing blank or null fields with
*               field specified in this parameter 
*  2007-08-01 - Added checking of date field such that if a non-date field is treated as a date field in the
*               padspaces specification file, the program will alert the user that a non-date field is 
*               treated as a date field in the padspaces specification file 
 =================================================================================================
 */
...
public class JDBCSQLSelect {
    ...
    static final String ProgramVersion = "1.2";
    ...
    static public void main(String args[]) {
         ...
         System.out.println("Copyright (c) 2007, Washington Alto. All rights reserved.              ");
         ...
         if (args[0].equalsIgnoreCase("--help")) {
              ...
              System.out.println("             2nd parameter - If 1st parameter is either CHAR or NUMERIC,    ");
              System.out.println("                             then this parameter specifies the string length");
              System.out.println("                             of the field to be padded with spaces.         ");
              System.out.println("                             If 1st parameter is DATE, then this parameter  ");
              System.out.println("                             is either 'SALIENT' or 'REGULAR' where SALIENT ");
              System.out.println("                             means date is formated as 'mmddyy' while       ");
              System.out.println("                             REGULAR means date is formatted as 'mm/dd/yyyy'");
              System.out.println("                             If parameter is unspecified and 1st parameter  ");
              System.out.println("                             is DATE, then SALIENT date format is assumed   ");
              ...
              System.out.println("                             the numeric field. If unspecified, then 100 is ");
              System.out.println("                             assumed to be the multiplier                   ");
              System.out.println("                             If the 1st parameter is CHAR, then this        ");
              System.out.println("                             parameter validates the CHAR field as to the   ");
              System.out.println("                             ff. types: ALPHA, ALPANUMERIC, and NUMERIC. If ");
              System.out.println("                             the field fails validation, then an empty or   ");
              System.out.println("                             blank string is returned                       ");
              System.out.println("             4th parameter - If the 1st parameter is CHAR and 3rd parameter ");
              System.out.println("                             is specified, then this parameter replaces     ");
              System.out.println("                             field if it is either null or blank string     ");
              System.out.println("");
              return;
         } 
         JDBCURL           = args[0];
         ...
         PadSpaceFileArray = getFormatFileInput(PadSpaceFile,IsCSV);  // retrieves Pad Spaces Specification in string array from Pad Spaces Specs file
   ...
   /* ========================================================================= 
    ...
    static private void printResultSet(ResultSet rs, String OutputFile, boolean IsCSV, boolean HasHeader,String FieldDelimiter,String TextQualifier,int RowsCopiedIncrement,String[] PadSpaceFileArray) {
         ...
         boolean IsFieldNull = false;                                      // Indicator for determining if field is null
              ...
              for (i=0; rs.next();) {
                   for (int j = 1; j <= rsmd.getColumnCount(); j++) {
                        ...
                        try {
                             ...
                             IsFieldNull = rs.getString(j) == null ? true : false;
                        } catch (SQLException e1) {
                             System.err.println("SQL Exception in printResultSet method: "+e1.getMessage());
                             System.err.println("");
                             System.err.println("Stack Trace: ");
                        ...
                        if (IsPadSpaceFileExist && !IsCSV) {
                             try {
                                 fieldstring = padFieldNames (fieldstring,PadSpaceFileArray[j-1],i,rsmd.getColumnName(j),IsFieldNull); 
   ...
   /* ========================================================================= 
      ...
      ========================================================================= */
    static String padFieldNames (String inputFieldString,String padStrField,int rowNum,String FieldName,boolean IsFieldNull) {
         String outputString = inputFieldString;
         String[] arglist;
         ...
         try {
              if (arglist[0].trim().toUpperCase().equalsIgnoreCase("CHAR")) {
                    /* This is for character field handling */
                   if (arglist.length > 2) {  /* Handles char field validation whether it's ALPHA, ALPANUMERIC, NUMERIC */
                       if (arglist[2].trim().toUpperCase().equalsIgnoreCase("ALPHA")&&!inputFieldString.matches("[a-zA-Z\\s_,.]*")) {
                                System.out.println("Alert for row number "+String.format("%,d",rowNum+1)+": Field ["+FieldName+"] whose value is '"+inputFieldString+"' is replaced by a blank string ''. Expected CHAR type is 'ALPHA' whose regex matches '[a-zA-Z\\s_,.]*' ...");
                                inputFieldString = "";
                       } else if (arglist[2].trim().toUpperCase().equalsIgnoreCase("NUMERIC")&&!inputFieldString.matches("[0-9]*")) {
                                System.out.println("Alert for row number "+String.format("%,d",rowNum+1)+": Field ["+FieldName+"] whose value is '"+inputFieldString+"' is replaced by a blank string ''. Expected CHAR type is 'NUMERIC' whose regex matches '[0-9]*' ...");
                                inputFieldString = "";
                       } else if (arglist[2].trim().toUpperCase().equalsIgnoreCase("ALPHANUMERIC")&&!inputFieldString.matches("[a-zA-Z0-9\\s_]*")) {     
                                System.out.println("Alert for row number "+String.format("%,d",rowNum+1)+": Field ["+FieldName+"] whose value is '"+inputFieldString+"' is replaced by a blank string ''. Expected CHAR type is 'ALPHANUMERIC' whose regex matches '[a-zA-Z0-9\\s_]*' ...");
                                inputFieldString = "";
                       }    
                   }
                   if (arglist.length > 3) {  /* Replaces blank or null string with that specified in 4th parameter of pad spaces specs file */
                         if (inputFieldString.trim().length()==0) {
                              inputFieldString = IsFieldNull ? "<NULL>" : "'"+inputFieldString+"'";
                              System.out.println("Alert for row number "+String.format("%,d",rowNum+1)+": Field ["+FieldName+"] whose value is "+inputFieldString+" is replaced by a replacement string '"+arglist[3].trim()+"'.");
                              inputFieldString = arglist[3].trim();
                         }
                   }
                   outputString = rpadspace(inputFieldString,Integer.valueOf(arglist[1].trim()));
              } else if (arglist[0].trim().toUpperCase().equalsIgnoreCase("NUMERIC")) {
                    /* This is for numeric field handling */
                   if (arglist.length == 3) {          
                         outputString = nrpadspace(inputFieldString,Integer.valueOf(arglist[1].trim()),Integer.valueOf(arglist[2].trim()));
                    } else {
                         outputString = nrpadspace(inputFieldString,Integer.valueOf(arglist[1].trim()),100);
                    }
              } else if (arglist[0].trim().toUpperCase().equalsIgnoreCase("DATE")) {
                    /* This is for date field handling */
                    if (inputFieldString.length()<=10) { /* Output error message when a non-date field is treated as date in the PADSPACESSPECS file */
                         System.out.println("Alert for row number "+String.format("%,d",rowNum+1)+": Field ["+FieldName+"] whose value is "+inputFieldString+" is not a datetime field!");
                         outputString = rpadspace("",6);
                    } else if ((arglist.length == 1) || ((arglist.length > 1) && arglist[1].trim().toUpperCase().equalsIgnoreCase("SALIENT"))) {  
                         outputString = salientdate(inputFieldString,true);
                    } else if ((arglist.length > 1) && !arglist[1].trim().toUpperCase().equalsIgnoreCase("SALIENT")) {
                         outputString = salientdate(inputFieldString,false);
                    } else {
                    ...
                    System.err.println("Field Value                         : "+inputFieldString);
